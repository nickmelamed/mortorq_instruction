...
Recall from `ml_resources/deep_learning_primer` and `ml_resources/rl_primer`: a value network outputs one number (a Regression Head); a policy network outputs a probability distribution over actions (a Classification Head plus softmax). Neither is hallucination-proof: a value network trained on last season's data can be confidently wrong about a team whose robot changed significantly over the offseason, the same "confidently wrong" warning `ai_primer/00-ai-for-programming.md` gave about any model, and the same failure mode `08-guardrails-failure-modes-and-eval.md` covered for hallucinated tool results, just coming from a network's forward pass instead of a language model's text generation.
...
Score yourself the same honest way `12` asked you to: can you say, specifically, what this model-backed tool still gets wrong or doesn't handle. Don't just focus on that it works, but where its confidence should and shouldn't be trusted.

## Resources

- [On Calibration of Modern Neural Networks](https://arxiv.org/abs/1706.04599) - the original paper on neural network confidence calibration, directly behind this module's (and `rl_primer/02`'s) warning about an overconfident value/policy network (paper)
