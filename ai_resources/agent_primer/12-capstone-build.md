...
Your finished scouting agent should be a single, coherent system where each earlier module's piece is a visible, working part. Concretely, that means:
...
5. **Guardrails** (`08`): at least one guardrail should be demonstrable, not just described. This could be a cap on tool calls or cost, a check that rejects a hallucinated or missing result, or a defense against an injected instruction. \\
...
| Presentation is honest about limits | The presenter can say, specifically, what the agent still gets wrong or doesn't handle. Presentation should not just discuss what it does well |
