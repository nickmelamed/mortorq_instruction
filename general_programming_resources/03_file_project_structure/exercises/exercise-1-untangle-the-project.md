...
7. Name the grouping style your new `src/`/`tests/`/`data/` structure uses — by layer/type, or by feature (`concept.md`'s "Grouping files" section) — and write one or two sentences on whether this small, single-feature project would actually be better served by the other style, or worse, and why.
...
- [ ] I can name which grouping style (by-type or by-feature) my reorganized structure uses, and justify why the other style wouldn't suit this particular project better

## Reflection
Notice that reorganizing wasn't without consequence; moving `main.py` broke a hardcoded path, and you had to go fix it. That's a real, common cost of restructuring a project: file locations and the references between them are coupled (the same idea from `02_code_organization_modularization`), and moving one without checking the other silently breaks things. Naming your grouping style in step 7 is worth the extra sentence, too — "by-type" and "by-feature" stop being abstract labels the moment you have to defend one over the other for a project you just reorganized yourself.
