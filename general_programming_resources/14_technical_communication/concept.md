...
`04_documentation` already covers writing for someone who's going to open your code: a README, a docstring, a comment explaining a non-obvious *why*. That reader can always go check — if your comment is a little off, the code itself is the tiebreaker. This module is about a different reader, one who's never going to open the file at all: a mentor deciding what to prioritize, a judge scoring your notebook, a teammate reviewing a PR without three hours of context you have and they don't. For that reader, what you write or say *is* the whole picture — there's no code to fall back on if it's unclear, so the standard is higher, not lower.

Same underlying facts, different job depending on who's asking and what they need to do with the answer once you've told them.

## Abstraction is a dial, not a switch

The instinct when explaining something technical to a non-technical person is to either drown them in implementation detail or oversimplify until the explanation is just wrong. Neither is necessary — abstraction level is a dial you set deliberately, not a binary choice between "the whole truth" and "a lie for beginners." The technique: before you write or say anything, name the one decision or belief you need this specific reader to walk away with, then include only what actually serves that — nothing more, because more isn't more helpful, it's more to wade through.
...
Both are true. Neither is a lie for the audience it's aimed at. The second one just leaves out the exact constant name and the encoder-noise justification, because the judge's decision ("did this team think carefully about this tradeoff?") doesn't depend on that detail — it depends on knowing a deliberate tradeoff was made and why.

## Lead with the conclusion

Once you know what the reader needs to walk away with, say it *first*. The instinct is to narrate chronologically — what you tried, then what didn't work, then what you tried next, then finally the actual result — but a busy reader (a lead skimming a status update, a reviewer opening a PR, a judge with six more teams to see today) decides whether to keep reading in your first sentence. Put the outcome or the ask there, and let the chronology, if it matters at all, come after:
...
The second version front-loads the one sentence a skimming reader actually needs, then earns the rest of the reader's attention with the interesting part, instead of making them wait for it.

## A PR description is a communication artifact, not a checklist item

`git_resources/CONTRIBUTING.md` already requires a PR description covering **what changed, why, and how to test it** — that's the format. What it doesn't tell you is how to think about filling it in well, and it's easy to satisfy the letter of that checklist while still writing something useless:
...
## FRC's sharpest version: the engineering notebook and the judging interview
...
- **`04_documentation`** — the sibling skill, for the reader who *will* open your code. Read that module's "what vs. why" comment guidance before this one if you haven't — this module assumes it.
