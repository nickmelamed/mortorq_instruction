...
## Scenario
A teammate wrote `examples/broken_script.sh` to print a quick pre-practice summary: every open `TODO` in the code, and how many errors showed up in the last match log. They know from `notes/deploy_notes.txt` that there should be **two** TODOs. The script only finds one, and they can't figure out why — "I searched `src/*.java`, that should get every Java file in `src`."
...
3. Once you can explain *why* `src/*.java` misses one of the two TODO-containing files, fix `broken_script.sh` so it searches recursively instead — using either `grep -r` directly, or `find` piped into `grep`.
