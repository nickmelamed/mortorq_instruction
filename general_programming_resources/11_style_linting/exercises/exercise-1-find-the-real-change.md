...
## Goal
See, concretely, how inconsistent formatting turns a one-line logic change into a noisy diff — and how keeping formatting consistent keeps the diff limited to only what actually changed.

## Scenario
`examples/style_diff/` has three files. `v1_original.py` is the starting point. `v2_mixed_change.py` and `v3_clean_change.py` both make the *exact same* real change to it (rejecting non-integer scores) — but `v2` also happens to have had its quotes reformatted from single to double by someone's editor in the same pass, while `v3` doesn't.
...
## Reflection
`v2` and `v3` are functionally identical — same behavior change, same result for every input. The only difference is that `v2`'s diff forces a reviewer to do the mental work of separating "this line matters" from "this line just got reformatted," for every single changed line, while `v3`'s diff shows *only* what actually matters. Multiply that friction across a real pull request with dozens of files, and the value of running a formatter consistently — so this kind of noise never enters a diff in the first place — stops being a style preference and starts being a real, measurable cost to how fast and how carefully a team can review each other's work.
