...
`fake_tba_api.py`'s 4th call always raises `ConnectionError`, standing in for one dropped request during a live event. This is not a rare edge case, just what happens when you depend on venue wifi. `sync_scores.py` never expected that, so the whole script dies on a single bad call.
...
## Reflection
This is `frc_resources`' "no usable internet in the pits" constraint showing up somewhere that has nothing to do with CAN bus wiring; the exact same unreliable-network problem reaches any code that calls an API you don't control.