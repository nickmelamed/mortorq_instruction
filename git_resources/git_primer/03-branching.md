...
So, working on branches makes your life easy because you can experiment without worrying about messing up the original code. 
...
Sometimes you don't want to switch branches at all. Maybe you just want to grab one file's version from another branch. Maybe a teammate fixed something on `main` that you need in your feature branch, but you're not ready to merge everything else yet. 
...
This prints the file's contents from that branch straight to your terminal. Nothing gets staged or copied, since it's purely read-only. 
...
Git will refuse to delete a branch with `-d` if it has commits that haven't been merged anywhere yet. Don't worry, this is Git protecting you from losing work. If you are **certain** you want to delete a branch anyway (including any unmerged commits on it), use a capital `-D`: 

```bash
git branch -D BRANCH
```

Be careful with `-D`, as this really can throw away work that isn't saved anywhere else. If you need to recover a branch you deleted this way, `git reflog` (covered in `06-solving-common-issues.md`) is usually your way back. 
