...
public class OrchestratorExample {
    ...
    public static void main(String[] args) {
        ...
        // if this were reading real NetworkTables values instead.
    ...
    static void processDetection(DetectionResult detection) {
        ...
        if (isGamePiece && isConfident) {
            System.out.printf(
                "Game piece detected (confidence %.2f) -> running intake command%n", detection.confidence());
        } else if (isGamePiece) {
            System.out.printf(
                "Possible game piece (confidence %.2f) -> below threshold, ignoring%n", detection.confidence());
        } else {
            System.out.printf(
                "No game piece detected (confidence %.2f) -> continuing search%n", detection.confidence());
        }
    }
}
