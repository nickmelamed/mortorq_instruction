...
`00_why_three_languages` introduced dynamic typing at a high level. Recall that a Python variable's type is only checked while the program runs, not before. This topic goes one level deeper into what that buys you in practice, and why it's a big part of why nearly the entire machine learning world builds on Python.

## Duck typing

**Duck typing** is the practical consequence of dynamic typing, named after the somewhat humorous idea of "if it walks like a duck and quacks like a duck, it's a duck." In Python, when you call `some_object.some_method()`, Python doesn't check ahead of time whether `some_object`'s declared type promises to have `some_method`. This is because there is no such promise to check, because Python variables don't have declared types at all. Python just tries to call `some_method` on whatever `some_object` actually is, right then, and it works as long as that particular object happens to have a method by that name that behaves sensibly. It doesn't matter what class the object is, or whether it inherits from anything in particular; only whether it has the right method, with the right behavior, at the moment you call it.
...
`python.ipynb`'s duck typing demo calls one `average(values)` function on three completely unrelated types. Here's what it's actually calling into:

```mermaid
classDiagram
    class PlainList["list (built-in)"] {
        +__iter__()
        +__len__()
    }
...
No arrows. No shared base class, no shared interface — deliberately, unlike every diagram in `02_oop_inheritance`. `average()` just calls `sum(values)` and `len(values)` and trusts that whatever it was handed supports both. It happens that a `list`, a `numpy.ndarray`, and a `pandas.Series` all do, for entirely unrelated reasons, and Python never once checks for a common ancestor before allowing it.
...
```mermaid
flowchart TB
    py3["Python layer<br/>numpy / pandas / scikit-learn / PyTorch API<br/>~95% of what you actually write — fast to iterate"]
    native["C / C++ / CUDA layer<br/>the actual number-crunching inner loops<br/>~5% of the code, does ~100% of the heavy lifting"]
    py3 -->|"calls down into, for<br/>every real computation"| native
```

That's the second bullet above, drawn out: you're almost never actually waiting on Python itself.
...
`python.ipynb` walks through `numpy` and `pandas` basics, includes a small concrete duck typing demonstration, and ends with a minimal training loop using `scikit-learn` on a toy robotics dataset; the same kind of loop, just larger and on real data, that you'll build again in `05_capstone_pipeline`. That loop uses `scikit-learn`'s `LogisticRegression` rather than `PyTorch` deliberately: it's a good first model to reach for before anything heavier, and it keeps this topic's focus on the Python/numpy/pandas workflow itself rather than on a specific model library. `05_capstone_pipeline` picks `PyTorch` back up once the loop needs to produce a model you can actually export and run outside of Python.
