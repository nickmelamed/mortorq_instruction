...
2. `04a_cpp_intricacies` argued C++'s manual memory management matters for edge inference because of predictable timing and a tight memory budget. Robot orchestration code (Java, on the roboRIO — or its SystemCore successor starting the 2027 season) also runs on constrained hardware with a real time budget. Why does the tradeoff come out differently for that code than it does for the inference pipeline?
