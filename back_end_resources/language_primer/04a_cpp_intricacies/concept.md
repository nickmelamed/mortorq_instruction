...
**The heap** is a much larger pool of memory that your program can request chunks of manually, at any time, in any amount, and hang onto for as long as it wants, and is independent of which function is currently running. This is exactly what `FaultStack` needed: a node pushed inside one function call has to survive until some *other*, later function call pops it. That requirement — "this object must outlive the scope that created it" — is the signal that you need the heap, not the stack.

```mermaid
flowchart LR
    subgraph stackmem["Stack — scoped to the function call"]
        direction TB
        v1["talon : TalonMotor<br/>(02_oop_inheritance)<br/>destroyed automatically<br/>the instant its function returns"]
        v2["node : Node*<br/>just a pointer, a few bytes —<br/>this is what lives on the stack"]
    end

    subgraph heapmem["Heap — lives until something deletes it"]
        direction TB
        h1["Node{ message, next }<br/>(FaultStack::push)<br/>created by new — outlives push()'s<br/>own function call"]
    end

    v2 -->|points to| h1
```

`talon` is a plain stack object: no pointer involved, and it disappears the moment its scope ends, exactly as it did in `02_oop_inheritance`. `node`, from `FaultStack::push` in `03_data_structures_algorithms`, is different: `node` *itself* is a small pointer variable that lives on the stack, but what it points to — the actual `Node` — was created with `new` and lives on the heap, so it survives long after `push()` returns. That's the whole distinction this section is about: the pointer's home and the pointed-to object's home are two separate questions.
...
`FaultStack` in `03_data_structures_algorithms` is already an example of this idea: its destructor walks the linked list and `delete`s every node, so a `FaultStack` that goes out of scope cleans up all of its heap-allocated nodes by itself. RAII is what makes that guarantee dependable.
...
`smart_pointers_demo.cpp` makes this concrete with a function that throws partway through. Here's what happens to the two versions when that exception fires:

```mermaid
flowchart TB
    fail(["mightFail(true) throws"])

    fail --> raw["withRawPointerBuggy()"]
    raw --> rawskip["exception unwinds straight past<br/>'delete resource;' — it's never reached"]
    rawskip --> rawleak["Resource is never destroyed — LEAK"]

    fail --> uniq["withUniquePtr()"]
    uniq --> uniqdtor["stack unwinds through the<br/>local unique_ptr variable"]
    uniqdtor --> uniqsafe["unique_ptr's destructor runs<br/>automatically → Resource deleted, no leak"]
```

Nobody added a `catch` block to the `unique_ptr` version to make this work. RAII means the cleanup is tied to the variable going out of scope, and stack unwinding during an exception is still "going out of scope" — the destructor doesn't care *why* the function is exiting.
...
- **No garbage collector pauses.** Java's JVM (see `04c_java_intricacies`) periodically pauses your program to reclaim memory it can no longer reach. This usually happens for a few milliseconds, occasionally longer. That's a rounding error for robot orchestration logic running on a 20ms loop with slack to spare. It is not a rounding error for a vision pipeline that needs to hand off a result within a few milliseconds, every single frame, with no exceptions. C++ never does this, because memory is freed exactly when a `delete` (or a smart pointer's destructor) runs, on a schedule *you* control, not one an automatic collector decides on its own.
...
None of this makes C++ strictly "better" than Java or Python: it makes it the right tool for one specific job in this stack.
