...
## Goal
Add a second subscriber to `NetworkTablesDemo.java`'s `MockNetworkTable` that only reacts when a detection is confident enough to act on. This is the same shape as a robot deciding "don't auto-align to a target unless we're sure it's real."
