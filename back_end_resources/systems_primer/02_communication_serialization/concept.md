...
A competition robot is not one program running in one place. It's several: code on the roboRIO, a driver station laptop running the FRC Driver Station application, and, on some teams, a coprocessor running vision or ML code, all talking to each other constantly, over several different channels, using several different formats. This topic is about those channels and formats, which make up the robot's own internal "backend."
...
```text
    ...
    CAN bus                    Serial                NetworkTables
    (shared bus, every    (one device, one       (pub/sub, any number
    device addressed)     stream of bytes,       of listeners; publisher
         |                no addressing)          never knows who)
         |                     |                        |
         v                     v                        v
  +-------------+      +----------------+      +----------------------+
  |    motor    |      |   a sensor or  |      |    Driver Station     |
  | controllers |      |  a coprocessor |      | (joysticks in, tele-  |
  | (Talons,    |      |  not on CAN    |      | metry out) and, often,|
  |  Sparks...) |      |                |      | a coprocessor (vision |
  +-------------+      +----------------+      | results in)           |
                                                +----------------------+
```

Every motor controller reports status back over the same CAN bus it takes commands on; `serial_demo.cpp` shows the outbound half of the Serial branch; `NetworkTablesDemo.java` demonstrates the NetworkTables branch directly.

**CAN bus** connects the roboRIO to motor controllers (Talons, Sparks, and similar) and some other devices. Every motor controller on the bus gets commands (set this power, hold this position) and reports status (current draw, temperature, encoder position) back, all over one shared physical bus. You'll mostly interact with CAN indirectly, through a motor controller's vendor library. For example, you call `setPower(0.5)` in Java, and that library handles turning it into an actual CAN message.

**Serial** is a simpler, direct connection, commonly used for sensors or a coprocessor that isn't on the CAN bus at all. It's a straightforward stream of bytes between two devices, no shared bus, no addressing scheme. `cpp/serial_demo.cpp` shows the shape of a serial write.

**NetworkTables** is WPILib's own publish/subscribe system. One side publishes a value under a named key (say, `"detector/label"`); any number of other programs can subscribe to that same key and get the value, continuously, without the publisher needing to know who's listening or how many listeners there are. This is how the roboRIO and the driver station exchange data (joystick values going one way, telemetry going the other), and it's also exactly how a coprocessor running a vision pipeline (like the C++ inference program from the languages primer's capstone) hands its results to your Java robot code. `java/NetworkTablesDemo.java` shows the publish/subscribe pattern directly.
