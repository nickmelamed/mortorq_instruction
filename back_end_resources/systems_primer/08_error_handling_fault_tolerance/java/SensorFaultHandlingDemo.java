...
//
// MockSensor is scripted to simulate exactly the failure scenarios
// concept.md describes: a disconnect (a missing reading) and a stale
// reading (present, but too old to trust). SafeSensorReader wraps those
// raw readings with a bounds/freshness check and a fallback to the last
// known-good value, instead of letting a null or stale value flow straight
// through to whatever would use it next (in a real system: a motor command).
//
...
public class SensorFaultHandlingDemo {
    ...
    public static void main(String[] args) {
        ...
        // A scripted sequence of raw readings: normal, normal, normal,
        // DISCONNECTED, STALE, then back to normal. A real sensor would
        // produce these unpredictably; scripting them here keeps the demo
        // reproducible and lets every failure mode show up on purpose.
        SensorReading[] script = {
            new SensorReading(45.0, 0),
            new SensorReading(46.2, 20),
            new SensorReading(47.0, 40),
            new SensorReading(null, 60),        // disconnected: no value at all
            new SensorReading(47.0, -160),       // stale: present, but far too old to trust
            new SensorReading(48.5, 100),
            new SensorReading(49.0, 120),
            new SensorReading(49.5, 140),
        };

        System.out.println("Reading a simulated arm-angle sensor across 8 ticks, with a "
            + "disconnect on tick 4 and a stale reading on tick 5:\n");
        ...
        System.out.println("\nNotice the robot never saw a null or a wildly stale value reach the "
            ...
            + "reading\" instead of propagating further.");
    }
}
