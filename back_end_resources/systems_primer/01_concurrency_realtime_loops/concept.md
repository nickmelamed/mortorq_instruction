...
## The 20-millisecond cycle of WPILib
...
20 milliseconds is not a suggestion. WPILib expects each loop iteration to happen inside that window. If one iteration runs long, the *next* tick is late too. Joystick input gets read late, motor output gets sent late, and if it happens badly enough or often enough, WPILib will report **loop overruns**, and your robot will visibly stutter or become unresponsive. In the worst case, code that never returns control to the loop at all just freezes the robot outright, live, in the middle of a match.

```text
Every tick is one lap around the loop: read -> decide -> act, then sleep out
whatever's left of the 20ms period before the next lap starts.

A normal tick (PeriodicLoopDemo.java, every tick):

    0ms                                                     20ms
    |---- work: ~1ms ----|---------- sleep: ~19ms ----------|
    tick starts                             tick ends, right on schedule

Tick 8 (BlockingCallBug.java) -- a 300ms blocking call replaces the work:

    0ms          20ms                                            300ms
    |----------------- blocked on the sensor read the whole time -----|
                 ^ 20ms budget exhausted here                     ^ tick 8 finally returns
                   OVERRUN by 280ms -- and every subsystem sharing
                   this loop (drivetrain, arm, intake) was frozen
                   for that entire 280ms, not just the slow call.

Ticks 7, 8, and 9 side by side. Everything before and after tick 8
looks completely normal, which is exactly what makes a one-off stall
easy to miss if you're not watching for it:

    tick 7: work took  1ms, sleeping 19ms to hold the 20ms period
    tick 8: work took 300ms, OVERRUN by 280ms! entire robot loop was stalled this whole time.
    tick 9: work took  1ms, sleeping 19ms to hold the 20ms period
```
