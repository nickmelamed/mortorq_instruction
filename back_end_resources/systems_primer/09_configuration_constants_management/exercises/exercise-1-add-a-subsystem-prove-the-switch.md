...
## Steps
1. Add a new nested class, `ClimberConstants`, following the exact shape of `IntakeConstants` (private constructor, `public static final` fields): give it a `MOTOR_CAN_ID` and a `CLIMB_POWER`. These don't need to depend on `ACTIVE_PROFILE`, just pick any reasonable constant values.
