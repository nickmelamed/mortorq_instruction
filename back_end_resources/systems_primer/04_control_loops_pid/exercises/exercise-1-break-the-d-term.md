...
2. Change the controller's construction line to `new SimplePIDController(0.06, 0.02, 0.0)` (removing the D term completely) and rerun. Compare the last several printed ticks to the original run: does the arm settle as cleanly, or does `error` swing between positive and negative for longer before settling?
