...
A PID controller looks at **error**. The error is defined as the difference between where you want to be (the *setpoint*) and where you actually are (the current sensor reading), and combines three terms, each reacting to a different aspect of that error, into one output value:
...
```text
   ...
   setpoint ----> [ error = setpoint - measurement ] <---- measurement
                                                                 ^
                                                                 |
                                              sensor reads the physical
                                              system the output just moved,
                                              feeding the next tick's error
```

Every tick runs this whole diagram once: read the sensor, compute error, run all three terms off that same error, sum them into one output, send it to the motor. Next tick, the sensor reading has changed because of what the motor just did, and the loop repeats. That closing arrow from "motor" back to "measurement" is what makes this a *feedback* loop, and it's exactly the periodic-loop shape from `01_concurrency_realtime_loops`, just drawn out term by term.

You are not expected to become a PID tuning expert from this paragraph, or this topic. Actually tuning `kP`, `kI`, and `kD` is saved for `frc_resources`. What you should walk away with is recognizing "read a sensor, compute an error, produce an output" as the same backend shape as everything else in this primer, just applied to a physical system instead of a data structure or a network message.

## Feedforward (briefly)

**Feedforward** is a related idea worth knowing the name of. Instead of waiting for error to show up and reacting to it (as P, I, and D all do), feedforward adds a term computed directly from what you already know about the system. For instance, "holding this arm up against gravity always needs at least this much output, regardless of current error." It's the difference between reacting after the fact and anticipating what you'll need before the error even appears. Real WPILib mechanism code frequently combines both: feedforward gets you close immediately, and PID cleans up whatever error is left.
